...
- **rules/analyzers** apply the heuristics shipped in `emailrecon/core.py`.
- **score** ranks by severity.
- **MCP server** (`emailrecon mcp`) exposes `scan` for Cognis.Studio agents.
