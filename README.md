...
[![PyPI](https://img.shields.io/pypi/v/cognis-emailrecon.svg?color=6b46c1)](https://pypi.org/project/cognis-emailrecon/) [![CI](https://github.com/cognis-digital/emailrecon/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/emailrecon/actions) [![License: COCL 1.0](https://img.shields.io/badge/License-COCL%201.0-2b6cb0.svg)](LICENSE) [![Suite](https://img.shields.io/badge/Cognis-Neural%20Suite-6b46c1.svg)](https://github.com/cognis-digital)
...
1. **Install** the CLI:
   ```bash
   pip install emailrecon
   ```

2. **Scan an email address** and its domain posture (MX, SPF/DMARC, breach hints):
   ```bash
   emailrecon scan alice@example.com
   ```

3. **Run fully offline** — skip all DNS lookups, and optionally check against a local breach corpus:
   ```bash
   emailrecon scan alice@example.com --no-dns --breach-corpus corpus.txt
   ```

4. **Read the output.** Add `--format json` for machine-readable results; tune `--timeout` for slow resolvers:
   ```bash
   emailrecon scan alice@example.com --timeout 3.0 --format json > recon.json
   ```

5. **Wire it into a pipeline** — feed the JSON into your enrichment or triage step:
   ```bash
   emailrecon scan alice@example.com --format json | jq '.domain'
   ```
...
```mermaid
flowchart LR
  IN[target / export] --> P[emailrecon<br/>collect + correlate]
