...
**emailrecon** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `emailrecon` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
emailrecon ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
emailrecon ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
emailrecon ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
emailrecon ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `emailrecon` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="emailrecon") for rec in records]   # records = your JSON output
