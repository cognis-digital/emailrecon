"""Native cognis-connect emit for emailrecon — forward findings to any platform.

Maps emailrecon's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    emailrecon ... --format json | emailrecon-emit --to stix
    emailrecon-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = f"{rec.get('domain')} - {rec.get('email')}"
        out['severity'] = 'info'
        if rec.get('cve'):
            out['severity'] = 'critical'
        elif rec.get('imo') or rec.get('mmsi'):
            out['severity'] = 'high'
        elif rec.get('lat') or rec.get('lon'):
            out['severity'] = 'medium'
        out['type'] = 'email'
        out['description'] = f"Email {rec.get('email')} found in domain {rec.get('domain')}"
        out['tags'] = []
        if rec.get('ipv4'):
            out['tags'].append('ipv4')
        if rec.get('sha256'):
            out['tags'].append('hash')
        if rec.get('cve'):
            out['tags'].append('cve')
        if rec.get('imo'):
            out['tags'].append('imo')
